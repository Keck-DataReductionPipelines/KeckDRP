...
class KpfPrimitives(CcdPrimitives, ImgmathPrimitives,
                     ProctabPrimitives, DevelopmentPrimitives):

    def __init__(self):
        super(KpfPrimitives, self).__init__()
    ...
    def subtract_bias(self):
        ...
        if len(tab) > 0:
            ...
            #    master = KeckDRP.KpfCCD.read(infile, unit='adu')
    ...
    def stack_biases(self):
        ...
        # create master bias
        if len(combine_list) >= KpfConf.MINIMUM_NUMBER_OF_BIASES:
            ...
            self.log.info('need %s biases to produce master' %
                          KpfConf.MINIMUM_NUMBER_OF_BIASES)
        self.write_proctab()
    ...
    def stack_darks(self):
        ...
        # create master bias
        if len(combine_list) >= KpfConf.MINIMUM_NUMBER_OF_DARKS:
            ...
            self.log.info('need %s darks to produce master' %
                          KpfConf.MINIMUM_NUMBER_OF_DARKS)
        self.write_proctab()
    ...
    def stack_internal_flats(self):
        ...
        # create master flat
        if len(combine_list) >= KpfConf.MINIMUM_NUMBER_OF_FLATS:
            ...
            self.log.info('need %s flats to produce master' %
                          KpfConf.MINIMUM_NUMBER_OF_FLATS)
        self.write_proctab()
